...
void monty_push(stack_t **stack, unsigned int line_number)
{
	...
	if (op_toks[1] == NULL)
	{
		err_set_op_tok(err_no_integer(line_number));
		return;
	}
	...
	if (check_mode_list(*stack) == STACK) /* STACK mode insert at front */
	{
		tmp = (*stack)->next;
		new->prev = *stack;
		new->next = tmp;
...
void monty_pint(stack_t **stack, unsigned int line_number)
{
	if ((*stack)->next == NULL)
	{
		err_set_op_tok(err_print(line_number));
		return;
	}
...
void monty_pop(stack_t **stack, unsigned int line_number)
{
	...
	if ((*stack)->next == NULL)
	{
		err_set_op_tok(err_pop(line_number));
		return;
	}
...
void monty_swap(stack_t **stack, unsigned int line_number)
{
	...
	{
		err_set_op_tok(err_short_stack(line_number, "swap"));
		return;
	}
